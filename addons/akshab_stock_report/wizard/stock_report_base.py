...
class AkshabStockReportBase(models.AbstractModel):
    ...
    aging_scope = fields.Selection([
        ('scope', 'منذ دخول النطاق المحدد (الشركة كاملة إذا لم تُحدد فروعاً/مواقع) — التحويلات الداخلية لا تُحتسب'),
        ('branch', 'منذ دخول الفرع — كل تحويل بين الفروع يبدأ عمراً جديداً في الفرع المستقبِل'),
        ...
        help='يحدد ما يُعد "استلاماً" في الأعمار وتواريخ أول/آخر استلام: على مستوى النطاق المحدد كاملاً '
             '(الافتراضي — تحويل بضاعة بين مستودعات الشركة لا يغيّر عمرها)، أو على مستوى كل فرع.')
