...
class AkshabStockReportBase(models.AbstractModel):
    ...
    warehouse_ids = fields.Many2many(
        'stock.warehouse', string='المستودعات / المستودعات',
        domain="[('company_id', '=', company_id)]",
        help='اتركه فارغاً لتضمين جميع المستودعات. يمكن اختيار أكثر من مستودع.')
    ...
    aging_scope = fields.Selection([
        ('scope', 'منذ دخول النطاق المحدد (الشركة كاملة إذا لم تُحدد مستودعات/مواقع) — التحويلات الداخلية لا تُحتسب'),
        ('branch', 'منذ دخول المستودع — كل تحويل بين المستودعات يبدأ عمراً جديداً في المستودع المستقبِل'),
    ], string='أساس احتساب العمر والاستلام', default='scope', required=True,
        help='يحدد ما يُعد "استلاماً" في الأعمار وتواريخ أول/آخر استلام: على مستوى النطاق المحدد كاملاً '
             '(الافتراضي — تحويل بضاعة بين مستودعات الشركة لا يغيّر عمرها)، أو على مستوى كل مستودع.')
