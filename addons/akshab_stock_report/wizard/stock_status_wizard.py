...
class AkshabStockStatusWizard(models.TransientModel):
    ...
    liquidation_discount = fields.Float(
        ...
        help='لتقدير السيولة المتوقعة من بيع الأصناف الراكدة (يُخفض تلقائياً حتى لا يقل السعر عن التكلفة).')
    ...
    show_transfers = fields.Boolean(
        string='إعادة التوزيع بين الفروع', default=True,
        help='اقتراحات نقل الأصناف الراكدة/البطيئة من فرع لا تُباع فيه إلى فرع تُباع فيه (يظهر عند وجود أكثر من فرع).')
    ...
    @api.constrains('stagnant_days', 'slow_cover_days', 'reorder_cover_days', 'liquidation_discount')
