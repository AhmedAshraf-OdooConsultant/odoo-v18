...
# how many products the "oldest stock" list of the aging report shows
OLDEST_TOP_N = 30

# Fixed display order of the product categories, as management reads them.  Matching is on the
# category name (the last part of the complete name), case- and space-insensitive; any category
# that is not listed keeps its place after the list, ordered by stock value then name.
CATEGORY_ORDER = [
    'Incense',
    'Fragrance Oils',
    'CRYSTAL',
    'Perfume',
    'Perfume Manufacturing Parts',
    'STONE',
    'SUBHA',
    'Premium Packaging',
...
_CATEGORY_RANK = {name.strip().lower(): i for i, name in enumerate(CATEGORY_ORDER)}


def category_rank(name):
    """Position of a category in the fixed order (unlisted categories come after all of them)."""
    if not name:
        return len(_CATEGORY_RANK)
    leaf = str(name).split('/')[-1].strip().lower()
    rank = _CATEGORY_RANK.get(leaf)
    if rank is None:                       # also try the full path (e.g. "All / Incense")
        rank = _CATEGORY_RANK.get(str(name).strip().lower())
    return rank if rank is not None else len(_CATEGORY_RANK)


def category_sort_key(name, value=0.0):
    """Sort key for category rows: fixed order first, then value (desc) then name."""
    return (category_rank(name), -(value or 0.0), str(name or ''))

ACTION_LABELS = {
    'liquidate': 'تصفية فورية',
    'discount_high': 'خصم قوي / عرض حزمة',
    'discount': 'عرض ترويجي / خصم',
    'transfer': 'إعادة توزيع بين المستودعات',
...
class StockReportEngine:
    ...
    def _load_scope(self):
        ...
        # an internal location that belongs to no warehouse is its own perimeter, shown by its
        # own name only (key = -location_id, never "without warehouse" nor the parent path)
        for loc in locations:
            if not loc.warehouse_id:
                self.wh_names[-loc.id] = loc.name
        ...
        # active products only: an archived product never appears in any report
        self.products = self.env['product.product'].search(prod_domain)
    ...
    def _sql_on_hand(self, at_date=None):
        ...
        if use_quants:
            self.cr.execute("""
                SELECT q.product_id, COALESCE(l.warehouse_id, -l.id), SUM(q.quantity)
                  FROM stock_quant q
                  JOIN stock_location l ON l.id = q.location_id
                 WHERE q.location_id IN %s AND q.product_id IN %s
              GROUP BY q.product_id, COALESCE(l.warehouse_id, -l.id)
            """, (tuple(self.location_ids), tuple(self.product_ids)))
        else:
            self.cr.execute("""
                SELECT product_id, wh, SUM(qty) FROM (
                    SELECT ml.product_id, COALESCE(l.warehouse_id, -l.id) AS wh, ml.quantity_product_uom AS qty
    ...
    def _sql_receipts(self, product_ids):
        ...
        * an unlinked return to a supplier reduces the latest receipts before it (LIFO).
        ...
        if scope:
            ...
            in_wh, out_wh = 'COALESCE(ld.warehouse_id, -ld.id)', 'COALESCE(ls.warehouse_id, -ls.id)'
            in_outside = "AND (ls.usage <> 'internal' OR COALESCE(ls.warehouse_id, -ls.id) <> COALESCE(ld.warehouse_id, -ld.id))"
            out_outside = "AND (ld.usage <> 'internal' OR COALESCE(ld.warehouse_id, -ld.id) <> COALESCE(ls.warehouse_id, -ls.id))"
    ...
    def _sql_receipt_dates(self):
        ...
        if scope:
            ...
            wh_col = 'COALESCE(ld.warehouse_id, -ld.id)'
            group_by = 'ml.product_id, ' + wh_col
            outside = "AND (ls.usage <> 'internal' OR COALESCE(ls.warehouse_id, -ls.id) <> COALESCE(ld.warehouse_id, -ld.id))"
    ...
    def _sql_sales(self, window_from, with_last_sale=True):
        ...
        self.cr.execute("""
            SELECT ml.product_id, COALESCE(ls.warehouse_id, -ls.id),
             ...
             WHERE m.state = 'done' AND m.date <= %s
               ...
               AND ml.product_id IN %s
          GROUP BY ml.product_id, COALESCE(ls.warehouse_id, -ls.id)
        ...
        self.cr.execute("""
            SELECT ml.product_id, COALESCE(ld.warehouse_id, -ld.id), SUM(ml.quantity_product_uom),
             ...
             WHERE m.state = 'done' AND m.date <= %%s
               ...
               AND ml.product_id IN %%s
          GROUP BY ml.product_id, COALESCE(ld.warehouse_id, -ld.id)
    ...
    def build(self):
        ...
        # locations without a warehouse that hold stock or had sales (key = -location_id)
        extra = {wh for (pid, wh), qty in on_hand.items() if wh < 0 and qty}
        extra |= {wh for (pid, wh), qty in sales.items() if wh < 0 and qty}
        wh_ids += sorted(extra, key=lambda wh: self.wh_names.get(wh, ''))
        self.wh_ids = wh_ids
    ...
    def _assemble(self, rows):
        ...
        def matrix(group_key, name_of, sort_key=None):
            ...
            out.sort(key=sort_key or (lambda x: -x['total']))
        ...
        aging_by_category = matrix(lambda r: [(r['category'], 1.0)], lambda k: k,
                                   sort_key=lambda x: category_sort_key(x['name'], x['total']))
        ...
        categories.sort(key=lambda c: category_sort_key(c['name'], c['value']))
        ...
        for r in rows:
            for t in r['transfers']:
                transfer_rows.append(dict(t, product=r['display_name'], code=r['code'], name_ltr=r['name_ltr'], category=r['category'],
        ...
        def group_by_category(items):
            ...
            for cname, grp in groups.items():
                out.append({'name': cname, 'name_ltr': bool(re.match(r'^[^\w]*[A-Za-z]', cname or '')),
                            ...
                            'sale_value': sum(r['sale_value'] for r in grp),
                            'expected_cash': sum(r['expected_cash'] for r in grp)})
            out.sort(key=lambda g: category_sort_key(g['name'], g['value']))
            return out
        ...
        for key, items in (('stagnant', stagnant_rows), ('slow', slow_rows), ('active', active_rows),
            ...
            status_groups[key] = {
                ...
                'sale_value': sum(r['sale_value'] for r in items),
                'sales_qty': sum(r['sales_qty'] for r in items),
        ...
        turnover_lists = {
            ...
            'none_sale_value': sum(r['sale_value'] for r in no_turnover_all),
        ...
        meta = {
            ...
            'warehouses_display': '، '.join(self.wh_names[wh] for wh in self.wh_ids if wh in self.wh_names and (wh > 0)) or 'جميع المستودعات',
            ...
            'aging_scope_label': (
                'منذ دخول %s — التحويلات الداخلية بين المستودعات/المواقع المحددة لا تُحتسب استلاماً ولا تبدأ عمراً جديداً'
                % ('الشركة' if not (o.warehouse_ids or o.location_ids) else 'النطاق المحدد (المستودعات/المواقع المختارة)')
            ) if o.aging_scope != 'branch' else
                'منذ دخول المستودع — التحويل بين المستودعات يُحتسب استلاماً ويبدأ عمراً جديداً في المستودع المستقبِل',
            'wh_aging_hint': (
                'توزيع تقريبي بنِسب كميات المستودعات — الأعمار محسوبة على مستوى النطاق كاملاً'
                if o.aging_scope != 'branch' else
                'أعمار كل مستودع من استلاماته الفعلية'),
        ...
        return {
            ...
            # top OLDEST_TOP_N by value in the LAST bucket ("more than X days")
            'oldest': sorted([r for r in stocked if r['bucket_value'][-1] > 0],
                             key=lambda r: (-(r['bucket_value'][-1]), -(r['max_age'] or 0)))[:OLDEST_TOP_N],
        }
    ...
    def _insights(self, kpi, status_by_key, buckets, by_warehouse, by_category, stagnant_rows, slow_rows,
        ...
        if len(by_warehouse) > 1:
            ...
            if worst['stagnant_value'] > 0:
                out.append('أعلى نسبة ركود في مستودع %s: %s من قيمة مخزونه (%s %s).' % (
        ...
        if transfer_rows:
            out.append('يمكن إعادة توزيع %s بين المستودعات (بقيمة %s %s) بدلاً من تخفيض الأسعار، لأنها تُباع في مستودع آخر.' % (
