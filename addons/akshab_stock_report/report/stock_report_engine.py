...
class StockReportEngine:
    ...
    def _sql_receipts(self, product_ids):
        """Receipts into each branch (from outside that branch), newest first, with returns
        applied to the ORIGINAL move they are linked to:
        ...
        * an unlinked return to a supplier reduces the latest receipts before it (LIFO).
        ...
        Returns {(product_id, wh_id): [entry, ...]} with entry = {'date', 'qty', 'move_id'}.
        """
        lines = defaultdict(list)
        if not self.location_ids or not product_ids:
            return lines
        params = (self.date_to, tuple(self.location_ids), tuple(product_ids))
        # 1) inbound lines into the branch from outside it
        self.cr.execute("""
            SELECT ml.product_id, COALESCE(ld.warehouse_id, 0), m.id, m.date, ml.quantity_product_uom,
             ...
             WHERE m.state = 'done' AND m.date <= %s
               ...
               AND (ls.usage <> 'internal' OR COALESCE(ls.warehouse_id, 0) <> COALESCE(ld.warehouse_id, 0))
        ...
        self.cr.execute("""
            SELECT ml.product_id, COALESCE(ls.warehouse_id, 0), m.origin_returned_move_id,
             ...
             WHERE m.state = 'done' AND m.date <= %s
               ...
               AND ml.product_id IN %s
               AND ml.quantity_product_uom > 0
               AND (ld.usage <> 'internal' OR COALESCE(ld.warehouse_id, 0) <> COALESCE(ls.warehouse_id, 0))
    ...
    def _sql_receipt_dates(self):
        """{(product_id, wh_id): (first_receipt, last_receipt)} over all history up to date_to
        (customer returns and linked returns are not receipts)."""
        res = {}
        if not self.location_ids or not self.product_ids:
            return res
        self.cr.execute("""
            SELECT ml.product_id, COALESCE(ld.warehouse_id, 0), MIN(m.date), MAX(m.date)
              FROM stock_move_line ml
              JOIN stock_move m ON m.id = ml.move_id
              JOIN stock_location ld ON ld.id = ml.location_dest_id
              JOIN stock_location ls ON ls.id = ml.location_id
        ...
        """, (self.date_to, tuple(self.location_ids), tuple(self.product_ids)))
    ...
    def _build_product_row(self, p, rounding, on_hand, sales, cov_sales, cogs, cogs_nolayer, last_sale,
        ...
        def add_age(take, age):
            nonlocal age_weighted, max_age, old_qty, year_qty
            bucket_qty[self._bucket_index(age)] += take
        ...
        for wh, q in wh_qty.items():
            ...
            remaining = q
            wh_weighted = 0.0
            for entry in receipts.get((p.id, wh), []):
    ...
    def _assemble(self, rows):
        ...
        # per warehouse: split the product's buckets pro-rata to the branch quantity
        aging_by_warehouse = matrix(
            lambda r: [(wh, (q / r['qty'])) for wh, q in r['wh_qty'].items() if r['qty']],
            lambda k: self.wh_names.get(k, ''))
