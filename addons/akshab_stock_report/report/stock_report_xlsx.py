...
class StockReportXlsx(AkshabXlsxBase):
    ...
    def _meta_rows(self):
        ...
        return [
            ...
            ('المستودعات', m['warehouses_display'], 'الفئات', m['categories_display']),
    ...
    def _sheet_summary(self):
        ...
        # 2. aging
        r = self._section(ws, r, '٢', 'أعمار المخزون', 'عمر الكمية منذ استلامها في المستودع (أساس FIFO)', span=6)
        ...
        # aging matrices
        for key, title in (('aging_by_warehouse', 'أعمار المخزون حسب المستودع (القيمة بالتكلفة)'),
            ...
            r += 1
            r = self._write_header(ws, r, [('الفئة' if 'category' in key else 'المستودع', 'txt')] +
        ...
        # 3. by warehouse
        r = self._section(ws, r, '٣', 'المخزون حسب المستودع', None, span=11)
        r = self._write_header(ws, r, [('المستودع', 'txt'), ('عدد الأصناف', 'c'), ('الكمية', 'c'), ('القيمة (%s)' % cur, 'c'),
                                       ('النسبة', 'c'), ('نشط (%s)' % cur, 'c'), ('بطيء الحركة (%s)' % cur, 'c'),
                                       ('راكد (%s)' % cur, 'c'), ('نسبة الراكد', 'c'), ('أصناف بلا بيع في المستودع', 'c'),
    ...
    def _product_headers(self, kind):
        ...
        base = [('الصنف', 'txt'), ('الفئة', 'txt'), ('الحالة', 'c')]
        std = self._std_headers()
        if kind == 'stagnant':
            cols = std[:2] + wh_cols + std[2:] + [
                ('آخر بيع', 'c'), ('أيام بلا بيع', 'c'), ('آخر استلام', 'c'),
            ...
            cols = std[:2] + wh_cols + std[2:] + [
                ('مبيعات الفترة', 'c'), ('متوسط البيع اليومي', 'c'),
            ...
            cols = [('مبيعات الفترة', 'c'), ('متوسط البيع اليومي', 'c'), ('آخر بيع', 'c'), ('آخر استلام', 'c'),
                    ('كمية قادمة', 'c'), ('تكلفة الوحدة (%s)' % cur, 'c'), ('سعر البيع (%s)' % cur, 'c'), ('التوصية', 'c'), ('تفصيل التوصية', 'txt')]
        else:  # active / new
            cols = std[:2] + wh_cols + std[2:] + [
                ('مبيعات الفترة', 'c'), ('متوسط البيع اليومي', 'c'),
    ...
    def _product_values(self, r, kind):
        ...
        base = [(r['display_name'], 'txtb'), (r['category'], 'txt'), (r['status_label'], 'badge:%s' % r['status'])]
        std = self._std_values(r)
        last_sale = r['last_sale_str']
        if kind == 'stagnant':
            cols = std[:2] + wh_vals + std[2:] + [
            ...
            cols = std[:2] + wh_vals + std[2:] + [
                (r['sales_qty'], 'qty'), (r['avg_daily'], 'dec'),
            ...
            cols = [(r['sales_qty'], 'qty'), (r['avg_daily'], 'dec'), (last_sale, 'c'), (r['last_receipt_str'], 'c'),
                    (r['incoming_qty'], 'qty'), (r['unit_cost'], 'money'), (r['price'], 'money'),
    ...
    def _sheet_products(self, name, rows, kind):
        ...
        widths = [44, 22, 14] + [14] * (len(headers) - 3)
        widths[-1] = 44
        ...
        if rows:
            ...
            # totals
            tot = [('الإجمالي: %d صنف' % len(rows), 'txt'), ('', 'blank'), ('', 'blank')]
            for i in range(3, len(headers)):
                k = all_vals[0][i][1]
                if k in ('qty', 'money', 'moneyb', 'moneyg', 'moneyr') and not headers[i][0].startswith(('تكلفة الوحدة', 'سعر البيع')):
    ...
    def _sheet_all_products(self):
        ...
        std = self._std_headers()
        headers = [('الصنف', 'txt'), ('الفئة', 'txt'), ('الحالة', 'c'), ('التوصية', 'c')] + std[:2] + wh_cols + std[2:] + [
        ...
        widths = [44, 22, 14, 18, 12, 10] + [12] * (len(headers) - 6)
        widths[-1] = 44
        ...
        for i, rw in enumerate(self.d['all_products']):
            std = self._std_values(rw)
            vals = [(rw['display_name'], 'txtb'), (rw['category'], 'txt'),
                    (rw['status_label'], 'badge:%s' % rw['status']), (rw['action_label'], 'badge:%s' % rw['action'])] + std[:2]
            if m['multi_warehouse']:
                vals += [(rw['wh_qty'].get(wh['id'], 0.0), 'qty') for wh in m['warehouses']]
            vals += std[2:] + [(rw['margin_pct'], 'pct'), (rw['sales_qty'], 'qty'), (rw['avg_daily'], 'dec'), (rw['cover_days'], 'int'),
