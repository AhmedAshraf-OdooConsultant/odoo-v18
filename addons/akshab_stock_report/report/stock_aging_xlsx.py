...
class StockAgingXlsx(AkshabXlsxBase):
    ...
    def _sheet_summary(self):
        ...
        if m['multi_warehouse'] and m['show_category_aging'] and d['aging_by_warehouse']['rows']:
            r = self._section(ws, r, '٣', 'أعمار المخزون حسب المستودع', 'القيمة بالتكلفة · %s' % m['wh_aging_hint'], span=n + 3)
            mat = d['aging_by_warehouse']
            r = self._write_header(ws, r, [('المستودع', 'txt')] + [(lbl, 'c') for lbl in mat['columns']] +
                                   [('الإجمالي', 'c'), ('نسبة القيمة %s' % last, 'c')])
        ...
        if d['oldest']:
            ...
            r = self._write_header(ws, r, [('الصنف', 'txt'), ('الفئة', 'txt')] + self._std_headers() +
                                          [('كمية %s' % last, 'c'), ('قيمة %s (%s)' % (last, cur), 'c'),
            ...
            for i, rw in enumerate(d['oldest']):
                ...
                self._write_row(ws, r, [(rw['display_name'], 'txtb'), (rw['category'], 'txt')] + self._std_values(rw) +
                                       [(rw['bucket_qty'][li], 'qty'), (rw['bucket_value'][li], 'moneyr'),
    ...
    def _sheet_products(self):
        ...
        headers = [('الصنف', 'txt'), ('الفئة', 'txt'), ('الكمية', 'c'), ('الوحدة', 'c')] + wh_cols + \
                  [('تكلفة الوحدة (%s)' % cur, 'c'), ('القيمة بالتكلفة (%s)' % cur, 'c'), ('سعر البيع (%s)' % cur, 'c'),
                   ('القيمة البيعية (%s)' % cur, 'c'), ('متوسط العمر (يوم)', 'c'), ('أقدم كمية (يوم)', 'c'),
                   ('أول استلام', 'c'), ('آخر استلام', 'c')] + self._bucket_headers() + \
                  [('نسبة القيمة %s' % last, 'c')]
        ws = self._new_sheet('الأصناف', [44, 22, 12, 10] + [13] * (len(headers) - 4))
        ws.set_row(0, 26)
        ...
        for c in d['by_category']:
            ...
            for i, rw in enumerate(rows):
                vals = [(rw['display_name'], 'txtb'), (rw['category'], 'txt'), (rw['qty'], 'qty'), (rw['uom'], 'c')]
        ...
        # grand total
        buckets = d['aging']['buckets']
        tot = [('الإجمالي العام', 'txt'), ('', 'blank'), (k['total_qty'], 'qty'), ('', 'blank')]
        if m['multi_warehouse']:
            tot += [(sum(rw['wh_qty'].get(wh['id'], 0.0) for rw in d['stocked']), 'qty') for wh in m['warehouses']]
        tot += [('', 'blank'), (k['total_value'], 'money'), ('', 'blank'), (k['total_sale_value'], 'money'),
                (k['avg_age'], 'int'), (k['max_age'], 'int'), ('', 'blank'), ('', 'blank')]
        for b in buckets:
            tot += [(b['qty'], 'qty'), (b['value'], 'money')]
        tot += [(k['last_pct'], 'pct')]
        self._write_total(ws, r, tot)
    ...
    def _sheet_method(self):
        m = self.m
        self._method_sheet([
            ('الكميات', 'كميات المخزون في المواقع الداخلية للمستودعات/المواقع المحددة حتى تاريخ التقرير (%s). الأصناف المؤرشفة لا تظهر في التقرير (الأصناف النشطة فقط).' % m['mode_label']),
            ('التقييم', '%s × الكمية.' % m['cost_basis_label']),
            ('أعمار المخزون', 'العمر %s. تُنسب الكمية الحالية إلى آخر استلامات دخلت النطاق بدءاً من الأحدث (FIFO)، ويُحسب عمر كل جزء من تاريخ استلامه، ثم يُوزع على الفئات العمرية: %s. المبيعات المسجلة دون رصيد لا تظهر في الأعمار، والاستلام اللاحق يغطيها أولاً ولا يُعمَّر من تاريخه إلا ما تبقى (شراء 100 ثم بيع 200 ثم شراء 200 ← يُعمَّر 100 فقط من تاريخ الشراء الثاني). الكمية التي لا يقابلها استلام مسجل تُنسب لأقدم استلام معروف.' % (m['aging_scope_label'], ' · '.join(m['bucket_labels']))),
            ('المرتجعات', 'المرتجع المرتبط بحركته الأصلية يُعالج على الحركة الأصلية وبتاريخها: مرتجع المشتريات يخفض كمية الاستلام الأصلي نفسه، ومرتجع العميل أو المستودع يعيد الكمية بعمرها الأصلي ولا يُعد استلاماً جديداً. المرتجع للمورد غير المرتبط بحركة يخفض آخر الاستلامات السابقة له.'),
