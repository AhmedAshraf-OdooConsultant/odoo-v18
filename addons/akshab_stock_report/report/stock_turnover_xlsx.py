...
class StockTurnoverXlsx(AkshabXlsxBase):
    ...
    def _headers(self, first_cols):
        cur = self.cur
        return first_cols + [
            ('مخزون البداية (كمية)', 'c'), ('مخزون البداية (%s)' % cur, 'c'),
            ('مخزون النهاية (كمية)', 'c'), ('مخزون النهاية (%s)' % cur, 'c'),
    ...
    def _values(self, obj):
        return [(obj['opening_qty'], 'qty'), (obj['opening_value'], 'money'), (obj['qty'], 'qty'), (obj['value'], 'money'),
    ...
    def _totals(self, label, obj, blanks_before=0, blanks_after=0):
        return [(label, 'txt')] + [('', 'blank')] * blanks_before + [
            (obj['opening_qty'], 'qty'), (obj['opening_value'], 'money'), (obj['qty'], 'qty'), (obj['value'], 'money'),
    ...
    def _sheet_summary(self):
        ...
        for idx, title, hint, subset in lists:
            ...
            r = self._write_header(ws, r, [('الصنف', 'txt'), ('المرجع', 'c'), ('الفئة', 'txt'), ('الوحدة', 'c'), ('الكمية', 'c'),
                                           ('القيمة (%s)' % cur, 'c'), ('متوسط المخزون', 'c'), ('مبيعات الفترة', 'c'), ('COGS', 'c'),
            ...
            for i, rw in enumerate(subset):
                self._write_row(ws, r, [(rw['name'], 'txtb'), (rw['code'], 'c'), (rw['category'], 'txt'), (rw['uom'], 'c'), (rw['qty'], 'qty'),
                                        (rw['value'], 'money'), (rw['avg_inventory'], 'money'), (rw['sales_qty'], 'qty'), (rw['cogs'], 'money'),
    ...
    def _sheet_products(self):
        d, m = self.d, self.m
        headers = self._headers([('الصنف', 'txt'), ('المرجع الداخلي', 'c'), ('الفئة', 'txt'), ('الوحدة', 'c')]) + [('آخر بيع', 'c'), ('آخر استلام', 'c')]
        ws = self._new_sheet('الأصناف', [40, 14, 22, 10] + [14] * (len(headers) - 4))
        ws.set_row(0, 26)
        ...
        for c in d['categories']:
            ...
            for i, rw in enumerate(rows):
                self._write_row(ws, r, [(rw['name'], 'txtb'), (rw['code'], 'c'), (rw['category'], 'txt'), (rw['uom'], 'c')] + self._values(rw) +
                                [(rw['last_sale_str'], 'c'), (rw['last_receipt_str'], 'c')], i % 2 == 1)
                r += 1
            self._write_total(ws, r, self._totals('إجمالي %s' % c['name'], c, blanks_before=3, blanks_after=2))
            r += 1
        self._write_total(ws, r, self._totals('الإجمالي العام', self._kpi_as_obj(), blanks_before=3, blanks_after=2))
        ws.freeze_panes(first, 1)
    ...
    def _sheet_lists(self):
        ...
        tl = d['turnover_lists']
        headers = [('الصنف', 'txt'), ('المرجع الداخلي', 'c'), ('الفئة', 'txt'), ('الوحدة', 'c'), ('مخزون البداية (كمية)', 'c'),
                   ('مخزون النهاية (كمية)', 'c'), ('القيمة (%s)' % cur, 'c'), ('تكلفة الوحدة', 'c'), ('آخر بيع', 'c'),
                   ('أيام بلا بيع', 'c'), ('آخر استلام', 'c'), ('أقدم كمية (يوم)', 'c')]
        ws = self._new_sheet('بلا دوران', [40, 14, 22, 10] + [14] * (len(headers) - 4))
        ws.set_row(0, 26)
        ...
        for i, rw in enumerate(tl['none_all']):
            self._write_row(ws, r, [(rw['name'], 'txtb'), (rw['code'], 'c'), (rw['category'], 'txt'), (rw['uom'], 'c'),
                                    (rw['opening_qty'], 'qty'), (rw['qty'], 'qty'), (rw['value'], 'moneyb'), (rw['unit_cost'], 'money'),
                                    (rw['last_sale_str'], 'c'), (rw['days_since_sale'], 'int'), (rw['last_receipt_str'], 'c'),
                                    (rw['max_age'], 'int')], i % 2 == 1)
            r += 1
        self._write_total(ws, r, [('الإجمالي — %s' % tl['none_items'], 'txt'), ('', 'blank'), ('', 'blank'), ('', 'blank'), ('', 'blank'),
                                  (tl['none_qty'], 'qty'), (tl['none_value'], 'money'), ('', 'blank'), ('', 'blank'), ('', 'blank'),
                                  ('', 'blank'), ('', 'blank')])
        if tl['none_all']:
            ws.autofilter(first - 1, 0, r - 1, len(headers) - 1)
        ws.freeze_panes(first, 1)
