...
class AkshabXlsxBase:
    ...
    def _formats(self):
        ...
        self.f_txt = f(border=1, border_color='#ECE7DB', align='right')
        self.f_txt_z = f(border=1, border_color='#ECE7DB', align='right', bg_color=BEIGE)
        self.f_txt_b = f(border=1, border_color='#ECE7DB', align='right', bold=True)
        self.f_txt_b_z = f(border=1, border_color='#ECE7DB', align='right', bold=True, bg_color=BEIGE)
    ...
    def _sheet_transfers(self):
        cur = self.cur
        headers = [('الصنف', 'txt'), ('الفئة', 'txt'), ('من فرع', 'txt'), ('الكمية فيه', 'c'), ('أيام بلا بيع فيه', 'c'),
                   ('إلى فرع', 'txt'), ('الكمية هناك', 'c'), ('متوسط البيع اليومي هناك', 'c'), ('تغطيته هناك (يوم)', 'c'),
                   ('الكمية المقترح نقلها', 'c'), ('قيمتها بالتكلفة (%s)' % cur, 'c')]
        ws = self._new_sheet('إعادة التوزيع', [38, 22, 20, 12, 14, 20, 12, 16, 14, 16, 18])
        ws.set_row(0, 26)
        ws.merge_range(0, 0, 0, 8, 'إعادة التوزيع بين الفروع', self.f_title)
        ws.merge_range(1, 0, 1, 8, 'مخزون بلا حركة في فرع بينما يُباع في فرع آخر — النقل بديل عن التخفيض', self.f_hint)
