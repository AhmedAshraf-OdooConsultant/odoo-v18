        ...
        self.env.cr.execute(f"""
            CREATE OR REPLACE VIEW {self._table} AS (
                SELECT
                    ...
                    -- price_subtotal بالعملة الأساسية: نستخدم rate من res_currency مباشرةً
                    -line.balance * COALESCE(
                        (SELECT rc.rate FROM res_currency_rate rc
                         WHERE rc.currency_id = move.company_currency_id
                           AND rc.company_id = line.company_id
                           AND rc.name <= CURRENT_DATE
                         ORDER BY rc.name DESC LIMIT 1),
                        1.0
                    )                                                               AS price_subtotal,
