...
class AkshabPosSalesReportWizard(models.TransientModel):
    ...
    config_ids = fields.Many2many(
        'pos.config', string='قنوات البيع', required=True,
    ...
    def _clean_text(self, text):
        """تنظيف النصوص القادمة من قاعدة البيانات (اسم الشركة خاصةً) من
        محارف التحكم الاتجاهية غير المرئية (RLM/LRM/عزل اتجاهي...) ومن
        أشكال العرض العربية Presentation Forms - هذه المحارف لا يملك خط
        التقرير رموزاً لها فتظهر مربعات فارغة وحروفاً ناقصة في الـ PDF."""
        if not text:
            return text
        # NFKC: يحول أشكال العرض العربية (ﺷ ﺮ ﻛ...) إلى الحروف القياسية
        text = unicodedata.normalize('NFKC', text)
        # إزالة محارف التنسيق غير المرئية (فئة يونيكود Cf)
        return ''.join(
    ...
    def _invoice_customer_stats(self, orders, invoice_history,
                                advance=True):
        """حساب أعمدة تاريخ العميل الثلاثة بمنطق تسلسلي صحيح:

        - عميل جديد؟ = أول فاتورة في حياة العميل وقعت داخل فترة
          التقرير (لا توجد له فواتير مرحّلة قبل بداية الفترة) - قيمة
          على مستوى العميل تظهر في جميع صفوفه خلال الفترة.
        - تاريخ آخر فاتورة = تاريخ الفاتورة السابقة مباشرة لهذه
          الفاتورة (سواء قبل الفترة أو داخلها حتى لو بنفس اليوم)،
          باستبعاد الفاتورة نفسها.
        - الأيام منذ آخر فاتورة = تاريخ هذه الفاتورة - تاريخ آخر
          فاتورة (صفر إن كانت بنفس اليوم).

        يعيد: {order_id: (last_date, days, is_new)}
        """
        period_start = fields.Datetime.context_timestamp(
            self, self.date_from).date()
        report_move_ids = {
            o.account_move.id for o in orders if o.account_move}

        # آخر فاتورة قبل بداية الفترة (أساس «عميل جديد؟»)
        baseline = {}
        for pid, entries in invoice_history.items():
            for d, mid in entries:
                if d >= period_start:
                    break
                if mid in report_move_ids:
                    continue
                baseline[pid] = d  # القائمة مرتبة تصاعدياً فآخرها الأحدث
        prev_by_partner = dict(baseline)

        stats = {}
        # الفواتير تعالَج بترتيب وقوعها الفعلي (تاريخاً ووقتاً)
        for order in sorted(orders, key=lambda o: (o.date_order, o.id)):
            partner = order.partner_id
            if not partner:
                stats[order.id] = (None, None, None)
                continue
            odate = fields.Datetime.context_timestamp(
                self, order.date_order).date()
            prevd = prev_by_partner.get(partner.id)
            # فواتير داخل الفترة من خارج قائمة التقرير (كفواتير المتجر
            # الإلكتروني) بتاريخ أسبق من يوم هذه الفاتورة
            for d, mid in invoice_history.get(partner.id, ()):
                if d >= odate:
                    break
                if mid in report_move_ids or d < period_start:
                    continue
                if prevd is None or d > prevd:
                    prevd = d
            days = (odate - prevd).days if prevd else None
            stats[order.id] = (prevd, days, partner.id not in baseline)
            if advance:
                # هذه الفاتورة تصبح «آخر فاتورة» لما يليها زمنياً
                prev_by_partner[partner.id] = odate
        return stats
    ...
    def _prepare_section_invoices(self, orders, invoice_history,
                                  refunds=False):
        # المرتجع ليس فاتورة فلا يُحدّث «آخر فاتورة» لما يليه
        stats = self._invoice_customer_stats(
            orders, invoice_history, advance=not refunds)
        rows = []
        for order in orders:
            last_inv, days, is_new = stats.get(
                order.id, (None, None, None))
            rows.append({
