...
class AkshabPosSalesReportWizard(models.TransientModel):
    ...
    def _get_old_partner_ids(self, partners):
        ...
        # لا يُعرض من الفاتورة أي بيانات، فقط نتيجة العدّ.
    ...
    def prepare_report_data(self):
        ...
        data['kpis'] = {
            ...
            'net_fmt': self._fmt_amount(gross - refunds),
    ...
    def _prepare_section_discounts(self, sales_orders, discount_product_ids):
        ...
        for order in sales_orders:
            ...
            for line in order.lines:
                ...
                # 2) الخصم العام (Global Discount) - بند منتج الخصم القياسي
                elif line.product_id.id in discount_product_ids \
                        and line.price_subtotal_incl < 0:
                    cfg_vals['discounts'][GENERAL] = \
                        cfg_vals['discounts'].get(GENERAL, 0.0) \
                        + abs(line.price_subtotal_incl)
                # 3) الخصم اليدوي (نسبة يضيفها الكاشير على البند)
                elif line.discount:
