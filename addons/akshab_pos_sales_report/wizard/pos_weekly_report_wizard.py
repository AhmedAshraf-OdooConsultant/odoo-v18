...
class AkshabPosWeeklyReportWizard(models.TransientModel):
    ...
    def prepare_weekly_report_data(self):
        ...
        data['matrix'] = self._weekly_matrix(sales_orders)
        data['top_invoices'] = {
        ...
        data['category_summary'] = self._weekly_category_summary(
            data['section4'])
        data['comparison'] = self._weekly_comparison(
            data, discount_product_ids)
    ...
    def _weekly_daily_breakdown(self, sales_orders, refund_orders,
        ...
        best_total = max((v['total'] for v in days.values()), default=0.0)
        rows = []
        for d in sorted(days):
            ...
            rows.append({
                ...
                'is_best': best_total > 0 and vals['total'] == best_total,
            })
        t_total = sum(v['total'] for v in days.values())
    ...
    def _weekly_matrix(self, sales_orders):
    ...
    # --- القسم الأسبوعي 2: مقارنة بالأسبوع السابق ------------------------
    def _weekly_comparison(self, data, discount_product_ids):
        prev_from = self.date_from - timedelta(days=7)
        prev_to = self.date_to - timedelta(days=7)
        ...
        prev_sales = prev_orders.filtered(lambda o: o.amount_total >= 0)
        prev_refunds = prev_orders - prev_sales
        p_gross = sum(prev_sales.mapped('amount_total'))
        p_refunds = abs(sum(prev_refunds.mapped('amount_total')))
        p_count = len(prev_sales)
        p_customers = len(set(prev_sales.mapped('partner_id').ids))
        p_discounts = 0.0
        for order in prev_sales:
            p_discounts += sum(
                amount for _name, amount in
                self._iter_order_discounts(order, discount_product_ids))

        c_gross = data['section1']['grand_total']
        c_refunds_total = abs(data['section3']['total'])
        c_count = data['section1']['total']['count']
        c_customers = data['section1']['total']['customers']
        c_discounts = data['section6']['grand_total']

        metrics = [
            ('إجمالي المبيعات (ر.س)', c_gross, p_gross, True),
            ('صافي المبيعات (ر.س)',
             c_gross - c_refunds_total, p_gross - p_refunds, True),
            ('عدد الفواتير', c_count, p_count, False),
            ('متوسط الفاتورة (ر.س)',
             c_gross / c_count if c_count else 0.0,
             p_gross / p_count if p_count else 0.0, True),
            ('عدد العملاء', c_customers, p_customers, False),
            ('إجمالي الخصومات (ر.س)', c_discounts, p_discounts, True),
        ]
        rows = []
        for label, cur, prev, is_amount in metrics:
            diff = cur - prev
        ...
        return {
            'rows': rows,
            'prev_from': self._fmt_dt(prev_from),
            'prev_to': self._fmt_dt(prev_to),
        }
