...
class AkshabPosWeeklyReportWizard(models.TransientModel):
    ...
    def _weekly_trend_and_comparison(self, data, discount_product_ids):
        ...
        cur = {
            'gross': data['section1']['grand_total'],
        ...
        for i in range(back + 1):
            ...
            trend_rows.append({
                ...
                'total_fmt': self._fmt_amount(gross),
                'count': count,
                'avg_fmt': self._fmt_amount(gross / count if count else 0.0),
                'customers': customers,
        ...
        metrics = [
            ...
            ('صافي المبيعات (ر.س)',
             cur['gross'] - cur['refunds'], p_gross - p_refunds, True),
            ('عدد الفواتير', cur['count'], p_count, False),
            ('متوسط الفاتورة (ر.س)',
             cur['gross'] / cur['count'] if cur['count'] else 0.0,
