...
class AkshabPosMonthlyReportWizard(models.TransientModel):
    ...
    # ------------------------------------------------------------------
    # هوكات الفترة والتسميات (يعاد تعريفها في تقرير الفترة)
    # ------------------------------------------------------------------
    REPORT_LABELS = {
        'report_title': 'تقرير المبيعات الشهري',
        'report_subtitle': 'MONTHLY SALES REPORT',
        'cur': 'هذا الشهر',
        'prev': 'الشهر السابق',
        'during': 'خلال الشهر',
        'vs_prev': 'عن الشهر السابق',
        'period_row': 'الشهر',
        'total_cur': 'إجمالي الشهر',
        'summary_title': 'الملخص التنفيذي — أبرز نتائج الشهر',
        'cmp_title': 'مقارنة بالشهر السابق',
        'ch_cmp_title': 'مقارنة مبيعات قنوات البيع بالشهر السابق — صافي المبيعات',
        'exec_title': 'مقارنة أداء قنوات البيع بالشهر السابق — الفئات والمنتجات',
        'trend_title': 'اتجاه آخر 4 أشهر (جميع القنوات)',
        'trend_chart_title': 'اتجاه صافي المبيعات - آخر 4 أشهر (جميع القنوات)',
        'trend_col': 'الشهر',
        'matrix_title': 'صافي المبيعات الأسبوعية حسب قناة البيع',
        'best_bucket': 'أفضل أسبوع في الشهر',
        'growth_hl': 'نمو صافي المبيعات عن الشهر السابق',
        'top_inv_title': 'أعلى 10 فواتير في الشهر (قنوات نقاط البيع)',
        'inv_total': 'إجمالي فواتير الشهر (جميع القنوات)',
        'top_products_title': 'أعلى 30 منتجاً مبيعاً خلال الشهر (جميع القنوات)',
        'products_total': 'إجمالي المنتجات المباعة خلال الشهر',
        'vip_title': 'عملاء VIP — الأعلى إنفاقاً خلال الشهر لكل قناة بيع',
        'vip_total': 'إجمالي عملاء الشهر المسجلين (قنوات نقاط البيع)',
        'online_note': 'لمقارنة قناة Online Store بالشهر السابق: اختر '
                       'فواتير المتجر الإلكتروني للشهر السابق في معالج '
                       'التقرير.',
    }

    def _labels(self):
        return dict(self.REPORT_LABELS)

    def _period_first_day(self):
        """أول يوم في فترة التقرير (أول الشهر في التقرير الشهري)."""
        return self.month_start.replace(day=1)

    def _period_label(self):
        first = self._period_first_day()
        return '%s %s' % (self.ARABIC_MONTHS[first.month - 1], first.year)

    def _bucket_title(self, index, b_from, b_to):
        """عنوان صف الدلو الزمني في جدول الأداء (أسبوع في الشهري)."""
        return 'الأسبوع %d: من %s إلى %s' % (
            index, b_from.strftime('%Y-%m-%d'), b_to.strftime('%Y-%m-%d'))

    def _bucket_header(self, index, b_from, b_to):
        """رأس عمود الدلو الزمني في المصفوفة."""
        return {'title': 'أسبوع %d' % index,
                'range_fmt': '%s - %s' % (b_from.strftime('%m-%d'),
                                          b_to.strftime('%m-%d'))}

    def _prev_period_bounds(self):
        """حدود فترة المقارنة (UTC): الشهر السابق في التقرير الشهري."""
        first = self._period_first_day()
    ...
    def _extra_channel_nets(self):
        """قنوات فواتير إضافية (مثل B2B في تقرير الفترة) تظهر في مقارنة
        مبيعات القنوات فقط دون باقي أقسام التقرير."""
        return []
    ...
    def prepare_monthly_report_data(self):
        ...
        data['L'] = self._labels()
        first = self._period_first_day()
        data['month_label'] = self._period_label()
    ...
    def _monthly_weekly_breakdown(self, sales_orders, refund_orders,
        ...
        for i, ((w_from, w_to), b) in enumerate(zip(weeks, buckets),
                                                start=1):
            rows.append({
                'label': self._bucket_title(i, w_from, w_to),
    ...
    def _monthly_matrix(self, sales_orders, refund_orders):
        weeks = self._month_weeks()
        headers = [
            self._bucket_header(i, w_from, w_to)
    ...
    def _prev_month_dataset(self, discount_product_ids):
        ...
        d_from, d_to = self._prev_period_bounds()
    ...
    def _channel_sales_comparison(self, online, prev, sales_orders,
        ...
        # قنوات فواتير إضافية (مثل B2B): تظهر في هذه المقارنة فقط
        for extra in self._extra_channel_nets():
            names.append(extra['name'])
            cur_vals.append(extra['cur'])
            prev_vals.append(extra['prev'])
        cur_vals.append(sum(cur_vals))
        prev_vals.append(sum(prev_vals))
        headers = names + ['الإجمالي']

        L = self._labels()
        row_cur = {'label': L['cur'], 'cells': []}
        row_prev = {'label': L['prev'], 'cells': []}
    ...
    def _channel_product_stats(self, orders, discount_product_ids):
        ...
        discount, products}}, 'products': {اسم المنتج: {...}}}} لقنوات
        نقاط البيع - بنفس منطق القسم الرابع (استبعاد بنود الخصم العام
    ...
    # --- القسم: تفاصيل جميع المنتجات المباعة حسب الفئة -------------------
    ...
    def _build_charts(self, data):
        ...
        trend_chart = [{
            'label': r.get('chart_label') or r['label'].split(' ')[0],
    ...
    def _executive_summary(self, data):
        ...
        L = self._labels()
        perf, channels, cats_g, customers = [], [], [], []
        ...
        if growth and growth['previous_fmt'] != '—' \
            ...
            perf.append({
                'text': '%s صافي المبيعات بنسبة %s %s، '
                        'بفارق قدره %s ر.س.' % (
                            verb, growth['pct_fmt'], L['vs_prev'],
                            growth['diff_fmt']),
                'cls': growth['class']})
        ...
        if ups:
            ...
            lead = drivers(top)
            cats_g.append({
                'text': 'حققت فئة %s أعلى نمو بنسبة %s %s%s.' % (
                            top['name'], top['pct_fmt'], L['vs_prev'],
        ...
        if new_c:
            customers.append({
                'text': 'انضم %s جديداً %s عبر قنوات نقاط '
                        'البيع.' % (self._ar_count(
                            new_c, 'عميل واحد', 'عميلان',
                            'عملاء', 'عميلاً'), L['during']),
                'cls': 'ak-up'})
        vip = data.get('vip') or {}
        if vip.get('vip_unique_count'):
            customers.append({
                'text': 'تجاوز إنفاق %s حد %s ر.س %s - '
    ...
    def _dashboard_highlights2(self, data):
        ...
        if best_week:
            items.append({
                'label': self._labels()['best_bucket'],
