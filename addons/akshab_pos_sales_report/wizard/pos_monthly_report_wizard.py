...
class AkshabPosMonthlyReportWizard(models.TransientModel):
    ...
    def prepare_monthly_report_data(self):
        ...
        data['vip'] = self._monthly_vip(
            sales_orders, old_ids, prev, online)
        ...
        data['price_changes'] = self._price_changed_products(data, prev)
    ...
    PRICE_CHANGE_MIN_PCT = 0.5  # أدنى نسبة تغير تُعتبر تغيراً في السعر

    def _price_changed_products(self, data, prev):
        """المنتجات التي تغيّر متوسط سعر وحدتها الفعلي (الإجمالي ÷
        الكمية) عن الشهر السابق فقط - المنتجات ثابتة السعر لا تظهر،
        والمنتجات الجديدة أو المتوقفة تُستبعد لعدم وجود سعر مرجعي.
        جميع القنوات (شاملة المتجر الإلكتروني)."""
        cur, cat_of = {}, {}
        for cat in data['section4']['categories']:
            for p in cat['products']:
                if self._is_discount_name(p['name']):
                    continue
                slot = cur.setdefault(
                    p['name'], {'qty': 0.0, 'total': 0.0})
                slot['qty'] += p.get('qty', 0.0)
                slot['total'] += p.get('total', 0.0)
                cat_of.setdefault(p['name'], cat['name'])
        prev_flat = {}
        for ch in (prev.get('channel_product_stats') or {}).values():
            for n, v in ch['products'].items():
                s = prev_flat.setdefault(n, {'qty': 0.0, 'total': 0.0})
                s['qty'] += v['qty']
                s['total'] += v['total']

        rows = []
        for name, c in cur.items():
            p = prev_flat.get(name)
            if not p or c['qty'] <= 0 or p['qty'] <= 0:
                continue
            cur_price = c['total'] / c['qty']
            prev_price = p['total'] / p['qty']
            diff = cur_price - prev_price
            threshold = max(
                0.01, prev_price * self.PRICE_CHANGE_MIN_PCT / 100.0)
            ...
            rows.append({
                'name': name,
                'category': cat_of.get(name, 'غير مصنّف'),
                'qty_fmt': self._fmt_qty(c['qty']),
                'cur_price_fmt': self._fmt_amount(cur_price),
                'prev_price_fmt': self._fmt_amount(prev_price),
        ...
        return {'rows': rows, 'count': len(rows),
                'min_pct': '{:.1f}'.format(self.PRICE_CHANGE_MIN_PCT)}
    ...
    def _monthly_vip(self, sales_orders, old_partner_ids, prev, online):
        """عملاء VIP لكل قناة بيع على حدة (شاملة قناة المتجر
        الإلكتروني إن اختيرت فواتيرها): كل عميل تجاوز إنفاقه في القناة
        حد VIP_MIN_TOTAL، مع أعمدة المقارنة بالشهر السابق."""
    ...
    def _executive_summary(self, data):
        ...
        فئات المنتجات / العملاء / ملاحظات للإدارة) تُبنى آلياً من أرقام
        التقرير وتُعرض في الصفحة الأولى."""
        def raw(fmt):
            try:
        ...
        cats = [(c['name'], raw(c['pct_fmt']), c['pct_fmt'])
                for c in data['category_summary']['rows']
                if c.get('pct_fmt') not in (None, '—')
                and raw(c.get('prev_fmt'))]
        ups = [c for c in cats if '+' in c[2]]
        downs = [c for c in cats if '-' in c[2]]
        if ups:
            top = max(ups, key=lambda c: c[1])
            cats_g.append({
                'text': 'حققت فئة %s أعلى نمو بنسبة %s عن الشهر '
                        'السابق.' % (top[0], top[2]), 'cls': 'ak-up'})
        if downs:
            bot = max(downs, key=lambda c: c[1])
            cats_g.append({
                'text': 'تراجعت فئة %s بنسبة %s، ويُوصى بمراجعة '
                        'مخزونها وتسعيرها.' % (bot[0], bot[2]),
                'cls': 'ak-down'})

        new_c = data['section1']['total'].get('new')
        if new_c:
            customers.append({
                'text': 'انضم %s عميلاً جديداً خلال الشهر عبر قنوات '
                        'نقاط البيع.' % new_c, 'cls': 'ak-up'})
        common = data.get('common_customers') or {}
        if common.get('count'):
            customers.append({
                'text': 'يشتري %d عملاء من أكثر من قناة بيع - شريحة '
                        'ولاء مرشحة لبرامج مخصصة.' % common['count'],
                'cls': ''})

        pc = data.get('price_changes') or {}
        if pc.get('count'):
            notes.append({
                'text': 'تغيّر سعر %d منتجاً عن الشهر السابق - '
                        'التفاصيل في قسم المنتجات المتغيرة الأسعار.'
                        % pc['count'], 'cls': ''})
        if not notes:
            notes.append({
