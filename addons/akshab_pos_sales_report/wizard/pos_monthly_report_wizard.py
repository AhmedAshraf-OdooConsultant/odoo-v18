...
class AkshabPosMonthlyReportWizard(models.TransientModel):
        ...
        for m in moves:
            ...
            for line in m.invoice_line_ids:
                ...
                cslot = online_ch['categories'].setdefault(
                    cname, {'qty': 0.0, 'total': 0.0, 'discount': 0.0})
                cslot['qty'] += line.quantity
                cslot['total'] += line.price_total
                cslot['discount'] += ldisc
                pslot = online_ch['products'].setdefault(
    ...
    def _channel_product_month_comparison(self, orders,
        ...
        for name in names:
            ...
            cat_rows, cat_total = self._cpc_rows(
                c['categories'], p['categories'], prev_missing)
            prod_rows, _pt = self._cpc_rows(
                c['products'], p['products'], prev_missing,
                limit=self.EXEC_TOP_PRODUCTS)
            channels_out.append({
                'name': name,
                'categories': cat_rows,
    ...
    def _add_online_product_stats(stats, cats, channel_name):
        ...
        for cat in cats.values():
            slot = ch['categories'].setdefault(
                cat['name'], {'qty': 0.0, 'total': 0.0, 'discount': 0.0})
            slot['qty'] += cat['qty']
            slot['total'] += cat['total']
            slot['discount'] += cat['discount']
            for p in cat['products'].values():
                pslot = ch['products'].setdefault(
                    p['name'], {'qty': 0.0, 'total': 0.0, 'discount': 0.0})
                pslot['qty'] += p['qty']
                pslot['total'] += p['total']
    ...
    def _build_charts(self, data):
        ...
        # 4) أشرطة أعلى المنتجات
        prods = data['top_products']['rows'][:8]
        mxp = max((p.get('total', 0.0) for p in prods), default=0.0)
        prods_chart = [{
            'name': p['name'], 'value_fmt': p['total_fmt'],
            'width': '{:.1f}%'.format(
