...
class AkshabPosMonthlyReportWizard(models.TransientModel):
    ...
    def _prev_month_dataset(self, discount_product_ids):
        ...
        product_totals, category_totals = {}, {}
        if orders:
            p4 = self._prepare_section_products(
                orders, discount_product_ids)
            for cat in p4['categories']:
                category_totals[cat['name']] = category_totals.get(
                    cat['name'], 0.0) + cat['total']
                for p in cat['products']:
                    product_totals[p['name']] = product_totals.get(
                        p['name'], 0.0) + p['total']
        ...
        for m in moves:
            ...
            for line in m.invoice_line_ids:
                ...
                cname = (product.categ_id.display_name
                         if product and product.categ_id else 'غير مصنّف')
                product_totals[pname] = product_totals.get(
                    pname, 0.0) + line.price_total
                category_totals[cname] = category_totals.get(
                    cname, 0.0) + line.price_total
                if line.discount:
                    online_m['discounts'] += (
    ...
    def _channel_comparison(self, data, online, prev,
                            sales_orders, refund_orders, order_discounts):
        """جدول المقارنة الجديد: القنوات أعمدة + عمود الإجمالي، ولكل
        مؤشر (من إجمالي المبيعات إلى إجمالي الخصومات) أربعة صفوف:
        هذا الشهر / الشهر السابق / التغير / نسبة التغير."""
