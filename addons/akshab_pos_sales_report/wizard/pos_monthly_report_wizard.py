...
class AkshabPosMonthlyReportWizard(models.TransientModel):
    ...
    online_move_m2_ids = fields.Many2many(
        'account.move', relation='akshab_pos_monthly_wiz_m2_move_rel',
        string='فواتير المتجر الإلكتروني (قبل شهرين)',
        domain=[('move_type', 'in', ('out_invoice', 'out_refund')),
                ('state', '=', 'posted')],
        help='اختياري: فواتير قناة Online Store للشهر قبل السابق - '
             'ليشمل قسم اتجاه آخر 4 أشهر مبيعات المتجر الإلكتروني')
    online_move_m3_ids = fields.Many2many(
        'account.move', relation='akshab_pos_monthly_wiz_m3_move_rel',
        string='فواتير المتجر الإلكتروني (قبل ثلاثة أشهر)',
        domain=[('move_type', 'in', ('out_invoice', 'out_refund')),
                ('state', '=', 'posted')],
        help='اختياري: فواتير قناة Online Store قبل ثلاثة أشهر - '
             'ليشمل قسم اتجاه آخر 4 أشهر مبيعات المتجر الإلكتروني')
    ...
    def _apply_month(self):
        ...
        الكتابة تتم دفعة واحدة (write واحدة) وليس حقلاً حقلاً: الكتابة
        المتسلسلة كانت تجعل قيد الـ92 يوماً يفحص حالة وسطية (بداية الشهر
        المختار مع تاريخ نهاية قديم = تاريخ اليوم) فيرفض الطباعة لأي شهر
        يبعد عن اليوم أكثر من 92 يوماً."""
        ...
        for wizard in self:
            ...
            if isinstance(wizard.id, models.NewId) or not wizard.id:
                # سجل onchange افتراضي: تحديث بالقيم دون كتابة فعلية
                wizard.update(vals)
            else:
                wizard.write(vals)
    ...
    def prepare_monthly_report_data(self):
        ...
        if online:
            ...
            data['config_names'] = '%s، %s' % (
                data.get('config_names') or '', online['name']) \
                if data.get('config_names') else online['name']

        # الاتجاه يُحسب بعد دمج الأونلاين حتى يشمل الشهر الحالي فواتير
        # المتجر الإلكتروني، والأشهر السابقة من حقول المعالج الاختيارية
        ...
        # قائمة أعلى 10 فواتير: قنوات نقاط البيع فقط (فواتير المتجر
        # الإلكتروني تجميعية فلا تنافس في القائمة)، بينما صف الإجمالي
        # يشمل كل القنوات
        inv_count = data['section2']['count']
        inv_total = abs(data['section2']['total'])
        online_inv_count = 0
        if online:
            inv_count += online['count']
            online_inv_count = online['count']
            inv_total += online['gross']
        data['top_invoices'] = {
            'rows': data['section2']['rows'][:10],
            'count': inv_count,
            'online_count': online_inv_count,
            'total_fmt': self._fmt_amount(inv_total),
        }
        if online and online['ref_rows']:
            # مرتجعات المتجر الإلكتروني تدخل قائمة المرتجعات والإجمالي
            # (عرض القيمة موجبة كصفوف مرتجعات نقاط البيع)
            online_refs = [dict(r, total_fmt=self._fmt_amount(
                abs(r['total']))) for r in online['ref_rows']]
            data['section3']['rows'] = (
                list(data['section3']['rows']) + online_refs)
            data['section3']['rows'].sort(
                key=lambda r: abs(r['total']), reverse=True)
            data['section3']['count'] += len(online['ref_rows'])
            data['section3']['online_count'] = len(online['ref_rows'])
            data['section3']['total_fmt'] = self._fmt_amount(
                abs(data['section3']['total']) + online['refunds'])
        ...
        data['channel_comparison'] = self._channel_comparison(
            ...
            order_discounts)
        data['channel_sales_comparison'] = self._channel_sales_comparison(
            online, prev, sales_orders, refund_orders, order_discounts)
        data['channel_product_comparison'] = \
            self._channel_product_month_comparison(
                orders, discount_product_ids, online, prev)
        cur_unit_prices = self._collect_pos_unit_prices(
            sales_orders, discount_product_ids)
        if online:
            self._collect_online_unit_prices(
                self.online_move_ids.filtered(
                    lambda m: m.state == 'posted'),
                discount_product_ids, cur_unit_prices)
        data['price_changes'] = self._price_changed_products(
            cur_unit_prices, prev)
        ...
        # ---------- بيانات الرسوم البيانية والخلاصة التنفيذية ----------
        data['charts'] = self._build_charts(data)
        data['exec_summary'] = self._executive_summary(data)
        return data
    ...
    def _monthly_trend_and_comparison(self, data, discount_product_ids):
        ...
        # فواتير المتجر الإلكتروني للأشهر السابقة (حقول المعالج
        # الاختيارية) - تدخل صافي المبيعات وعدد الفواتير والخصومات،
        # بينما يبقى عدد العملاء لقنوات نقاط البيع فقط
        prev_online_fields = [self.online_move_prev_ids,
                              self.online_move_m2_ids,
                              self.online_move_m3_ids]
        for offset, moves in enumerate(prev_online_fields, start=1):
            idx = back - offset
            if idx < 0:
                continue
            b = buckets[month_firsts[idx]]
            moves = moves.filtered(
                lambda m: m.state == 'posted'
                and m.move_type in ('out_invoice', 'out_refund'))
            for m in moves:
                if m.move_type == 'out_refund':
                    b['refunds'] += m.amount_total
                    continue
                b['gross'] += m.amount_total
                b['count'] += 1
                for line in m.invoice_line_ids:
                    if line.display_type in (False, 'product') \
                            and line.discount:
                        b['discounts'] += (line.price_unit * line.quantity
                                           * line.discount / 100.0)
    ...
    def _online_dataset(self):
        ...
        # بنود منتجات الخصم العام تُستبعد من تجميعات المنتجات/الفئات
        # (نفس منطق القسم الرابع لقنوات نقاط البيع)
        discount_product_ids = self._get_global_discount_product_ids()
        # تتبع تسلسلي لآخر فاتورة داخل الشهر لكل عميل (فواتير متعددة
        # لنفس العميل: الأيام تُحسب من الفاتورة السابقة مباشرة)
        online_prev = {}
        for m in sales.sorted(key=lambda x: (x.invoice_date or x.date
            ...
            for line in m.invoice_line_ids:
                ...
                if product and product.id in discount_product_ids:
                    continue
            ...
            baseline = last_by_partner.get(partner.id) if partner else None
            # «عميل جديد؟» على مستوى العميل: لا فواتير قبل بداية الشهر
            is_new = not baseline
            prev_d = None
            if partner:
                prev_d = online_prev.get(partner.id) or (
                    fields.Date.to_date(baseline) if baseline else None)
            days, last = '', None
            if prev_d:
                days = str((d - prev_d).days) if d else '-'
                last = prev_d.strftime('%Y-%m-%d')
            if partner and d:
                online_prev[partner.id] = d
            inv_rows.append({
    ...
    def _merge_online_into_summary(self, data, online):
        ...
        # (عدد العملاء والجدد والقدامى: قنوات نقاط البيع فقط -
        #  عملاء فواتير المتجر الإلكتروني لا يدخلون في العدادات)
        ...
        # قسم مبيعات الموظفين (5) يبقى لقنوات نقاط البيع فقط - فواتير
        # المتجر الإلكتروني تظهر في جزء قناة Online Store التفصيلي
        if online['discounts']:
            d_rows = [{
    ...
    @staticmethod
    def _raw_discount(row):
        """القيمة الخام لخصم صف منتج/فئة: من المفتاح الخام إن وُجد
        وإلا من النص المنسق."""
        if 'discount' in row:
            return row['discount'] or 0.0
        try:
            return float(str(row.get('discount_fmt', '0')
                             ).replace(',', ''))
        except ValueError:
            return 0.0

    def _merge_online_products(self, section4, cats):
        """يدمج تجميعات فئات/منتجات الأونلاين (كمية ومبلغاً وخصماً) في
        بنية القسم الرابع الخام ويعيد احتساب إجمالياته."""
        ...
        for cat in cats.values():
            ...
            if row is None:
                ...
                disc = self._raw_discount(row) + cat['discount']
                row['qty'] = q
                row['total'] = tot
                row['discount'] = disc
                row['qty_fmt'] = self._fmt_qty(q)
                row['total_fmt'] = self._fmt_amount(tot)
                row['discount_fmt'] = self._fmt_amount(disc)
                ...
                for p in prods_fmt:
                    ...
                    if e is None:
                        ...
                        ed = self._raw_discount(e) + p['discount']
                        e['qty'], e['total'], e['discount'] = eq, et, ed
                        e['qty_fmt'] = self._fmt_qty(eq)
                        e['total_fmt'] = self._fmt_amount(et)
                        e['discount_fmt'] = self._fmt_amount(ed)
        ...
        td = sum(self._raw_discount(c) for c in section4['categories'])
        section4['total_qty_fmt'] = self._fmt_qty(tq)
        section4['total_fmt'] = self._fmt_amount(tt)
        section4['total_discount_fmt'] = self._fmt_amount(td)
    ...
    def _rebuild_weekly_rows(self, data, online):
        ...
        for i, row in enumerate(rows):
            ...
            if i < len(online['week_buckets']):
                ...
                row['count'] += b['count']
                # عدد العملاء يبقى لقنوات نقاط البيع فقط
            nets.append((row, add))
        ...
        # t['customers'] يبقى كما حسبه تفصيل الأسابيع (نقاط البيع فقط)
    ...
    def _prev_month_dataset(self, discount_product_ids):
        ...
        channel_product_stats = self._channel_product_stats(
        ...
        for m in moves:
            ...
            online_ch = channel_product_stats.setdefault(
                self.ONLINE_CHANNEL_NAME,
                {'categories': {}, 'products': {}})
            for line in m.invoice_line_ids:
                if line.display_type not in (False, 'product'):
                    continue
                ...
                cname = (product.categ_id.display_name
                         if product and product.categ_id else 'غير مصنّف')
                ldisc = (line.price_unit * line.quantity
                         * line.discount / 100.0) if line.discount else 0.0
                online_m['discounts'] += ldisc
                if product and product.id in discount_product_ids:
                    # بنود منتجات الخصم تُستبعد من تجميعات المنتجات
                    # والفئات (نفس منطق قسم المبيعات حسب الفئة)
                    continue
                cslot = online_ch['categories'].setdefault(
                    cname, {'qty': 0.0, 'total': 0.0,
                            'discount': 0.0, 'products': {}})
                cslot['qty'] += line.quantity
                cslot['total'] += line.price_total
                cslot['discount'] += ldisc
                for pmap in (cslot['products'], online_ch['products']):
                    pslot = pmap.setdefault(
                        pname, {'qty': 0.0, 'total': 0.0, 'discount': 0.0})
                    pslot['qty'] += line.quantity
                    pslot['total'] += line.price_total
                    pslot['discount'] += ldisc
        if moves:
            channel_metrics[self.ONLINE_CHANNEL_NAME] = online_m

        products_total, categories_total = {}, {}
        for ch in channel_product_stats.values():
            for n, v in ch['products'].items():
                products_total[n] = products_total.get(n, 0.0) + v['total']
            for n, v in ch['categories'].items():
                categories_total[n] = categories_total.get(
                    n, 0.0) + v['total']

        # آخر سعر وحدة مسجل في الشهر السابق (نقاط البيع + الأونلاين)
        unit_prices = self._collect_pos_unit_prices(
            sales, discount_product_ids)
        self._collect_online_unit_prices(
            moves, discount_product_ids, unit_prices)
        ...
        return {
            ...
            'channel_product_stats': channel_product_stats,
    ...
    def _channel_comparison(self, data, online, prev,
                            sales_orders, refund_orders, order_discounts):
        """جدول المقارنة على نمط التقرير الأسبوعي: صف لكل مؤشر (من
        إجمالي المبيعات إلى إجمالي الخصومات) على إجمالي كل القنوات،
        والأعمدة: هذا الشهر / الشهر السابق / التغير / نسبة التغير."""
        ...
        def totals(mapping):
            ...
            for n in names:
                ...
                # عدد العملاء بدون عملاء فواتير المتجر الإلكتروني
                if n != self.ONLINE_CHANNEL_NAME:
        ...
        for key, label, is_amount in self.COMPARISON_METRICS:
            ...
            cv = self._metric_val(cur_tot, key)
            pv = self._metric_val(prev_tot, key)
            c = self._cmp_vals(cv, pv, fmt=fmt)
            rows.append({
                'label': label,
                'current_fmt': fmt(cv),
                'previous_fmt': c['prev_fmt'],
                'diff_fmt': c['diff_fmt'],
                'pct_fmt': c['pct_fmt'],
                'class': c['cmp_class'],
            })
    ...
    # --- القسم ١: مقارنة مبيعات قنوات البيع بالشهر السابق ---------------
    def _channel_sales_comparison(self, online, prev, sales_orders,
                                  refund_orders, order_discounts):
        """صافي مبيعات كل قناة بيع (شاملة المتجر الإلكتروني) مقارنة
        بالشهر السابق: القنوات أعمدة + عمود الإجمالي، والصفوف: هذا
        الشهر / الشهر السابق / التغير / نسبة التغير."""
        names = [c.name for c in self.config_ids]
        cur = self._channel_metrics(
            sales_orders, refund_orders, order_discounts)
        if online:
            names.append(online['name'])
            cur[online['name']] = {'gross': online['gross'],
                                   'refunds': online['refunds']}
        prev_m = prev['channel_metrics']

        def net(m):
            return (m['gross'] - m['refunds']) if m else 0.0

        cur_vals = [net(cur.get(n)) for n in names]
        cur_vals.append(sum(cur_vals))
        prev_vals = [net(prev_m.get(n)) for n in names]
        prev_vals.append(sum(prev_vals))
        ...
        for i, (cv, pv) in enumerate(zip(cur_vals, prev_vals)):
            online_unknown = (
                online and not prev['has_online']
                and i < len(names) and names[i] == online['name'])
            c = self._cmp_vals(cv, pv, has_prev=not online_unknown)
            row_cur['cells'].append(
                {'text': self._fmt_amount(cv), 'class': ''})
            row_prev['cells'].append({'text': c['prev_fmt'], 'class': ''})
    ...
    # --- القسم التنفيذي: مقارنة فئات ومنتجات كل قناة بالشهر السابق -------
    EXEC_TOP_PRODUCTS = 30

    @staticmethod
    def _is_discount_name(name):
        """أسماء فئات/منتجات الخصم تُستبعد من القسم التنفيذي (نفس
        منطق قسم المبيعات حسب الفئة)."""
        n = (name or '').strip().lower()
        return 'discount' in n or 'خصم' in n or 'خصومات' in n

    def _cpc_rows(self, cur_map, prev_map, prev_missing, limit=None):
        """صفوف مقارنة (فئة أو منتج): لكل مقياس - الكمية المباعة /
        إجمالي المبيعات / إجمالي الخصم - هذا الشهر والشهر السابق
        ونسبة التغير، مع صف إجمالي جاهز."""
        names = set(cur_map)
        if not prev_missing:
            names |= set(prev_map)
        ordered = sorted(
            names,
            key=lambda n: cur_map.get(n, {}).get('total', 0.0),
            reverse=True)
        if limit:
            ordered = ordered[:limit]
        metric_fmts = (('qty', self._fmt_qty),
                       ('total', self._fmt_amount),
                       ('discount', self._fmt_amount))
        rows = []
        totals = {k: [0.0, 0.0] for k, _f in metric_fmts}
        for n in ordered:
            c = cur_map.get(n) or {}
            p = prev_map.get(n) or {}
            row = {'name': n}
            for key, fmt in metric_fmts:
                cv, pv = c.get(key, 0.0), p.get(key, 0.0)
                totals[key][0] += cv
                totals[key][1] += pv
                cmpv = self._cmp_vals(
                    cv, pv, has_prev=not prev_missing, fmt=fmt)
                row[key + '_cur'] = fmt(cv)
                row[key + '_prev'] = cmpv['prev_fmt']
                row[key + '_pct'] = cmpv['pct_fmt']
                row[key + '_cls'] = cmpv['cmp_class']
            rows.append(row)
        total_row = {'name': 'الإجمالي'}
        for key, fmt in metric_fmts:
            cv, pv = totals[key]
            cmpv = self._cmp_vals(
                cv, pv, has_prev=not prev_missing, fmt=fmt)
            total_row[key + '_cur'] = fmt(cv)
            total_row[key + '_prev'] = cmpv['prev_fmt']
            total_row[key + '_pct'] = cmpv['pct_fmt']
            total_row[key + '_cls'] = cmpv['cmp_class']
        return rows, total_row

    def _channel_product_month_comparison(self, orders,
                                          discount_product_ids,
                                          online, prev):
        """قسم الإدارة العليا: لكل قناة بيع - مقارنة فئات المنتجات
        (كاملة) وأعلى المنتجات بالشهر السابق: الكمية المباعة /
        إجمالي المبيعات / إجمالي الخصم."""
        cur = self._channel_product_stats(orders, discount_product_ids)
        names = [c.name for c in self.config_ids]
        if online:
            self._add_online_product_stats(
                cur, online['cats'], online['name'])
            names.append(online['name'])
        prev_stats = prev.get('channel_product_stats') or {}
        empty = {'categories': {}, 'products': {}}
        channels_out = []
        for name in names:
            c = cur.get(name) or empty
            p = prev_stats.get(name) or empty
            prev_missing = (online and name == online['name']
                            and not prev['has_online'])
            # استبعاد فئات/منتجات الخصم (نفس منطق قسم ٩)
            cur_cats = {n: v for n, v in c['categories'].items()
                        if not self._is_discount_name(n)}
            prev_cats = {n: v for n, v in p['categories'].items()
                         if not self._is_discount_name(n)}
            cat_rows, cat_total = self._cpc_rows(
                cur_cats, prev_cats, prev_missing)

            # أعلى المنتجات في القناة مقسمة على الفئات
            flat_cur = {n: v for n, v in c['products'].items()
                        if not self._is_discount_name(n)}
            flat_prev = {n: v for n, v in p['products'].items()
                         if not self._is_discount_name(n)}
            top_names = set(sorted(
                flat_cur, key=lambda n: flat_cur[n]['total'],
                reverse=True)[:self.EXEC_TOP_PRODUCTS])
            product_groups = []
            ordered_cats = sorted(
                cur_cats.items(),
                key=lambda kv: kv[1].get('total', 0.0), reverse=True)
            for cname, cvals in ordered_cats:
                cprods = {n: v for n, v in
                          (cvals.get('products') or {}).items()
                          if n in top_names}
                if not cprods:
                    continue
                sub_prev = {n: flat_prev[n] for n in cprods
                            if n in flat_prev}
                rows, _t = self._cpc_rows(
                    cprods, sub_prev, prev_missing)
            ...
            channels_out.append({
                ...
                'prev_missing': bool(prev_missing),
                'has_data': bool(cat_rows),
            })
        return {'channels': channels_out,
                'top_limit': self.EXEC_TOP_PRODUCTS}

    # --- القسم: المنتجات التي تغيّر سعرها عن الشهر السابق ----------------
    def _collect_pos_unit_prices(self, orders, discount_product_ids,
                                 prices=None):
        """آخر قيمة مسجلة لحقل «سعر الوحدة» في بنود طلبات نقاط البيع
        خلال الفترة لكل منتج (الخصومات لا تؤثر على هذا الحقل):
        {اسم المنتج: {'dt', 'price', 'qty', 'category'}}.

        معالجة الكومبو:
        - بنود المكونات تُستبعد نهائياً: سعرها في البند هو نصيب موزع
          من سعر الكومبو وليس سعر المنتج الحقيقي.
        - منتج الكومبو نفسه سعره غير مسجل في بنده (صفر غالباً)، فيُشتق
          سعره الحقيقي = مجموع قيم بنود مكوناته ÷ كمية الكومبو، وأي
          تغير فيه يظهر في التقرير."""
        prices = prices if prices is not None else {}
        has_combo = None
        for order in orders:
            lines = order.lines
            if has_combo is None:
                has_combo = bool(lines) and \
                    'combo_parent_id' in lines[0]._fields
            # مجموع قيم المكونات لكل سطر كومبو أب في هذا الطلب
            combo_sums = {}
            if has_combo:
                for line in lines:
                    parent = line.combo_parent_id
                    if parent:
                        combo_sums[parent.id] = combo_sums.get(
                            parent.id, 0.0) + line.price_subtotal_incl
            for line in lines:
                ...
                if line.product_id.id in discount_product_ids:
                    continue
                if has_combo and line.combo_parent_id:
                    continue  # مكوّن كومبو: سعره الموزع لا يُعتد به
                if line.id in combo_sums:
                    # منتج كومبو: السعر الحقيقي من مجموع مكوناته
                    qty = line.qty or 1.0
                    unit = (line.price_subtotal_incl
                            + combo_sums[line.id]) / qty
                else:
                    unit = line.price_unit
                if unit <= 0:
                    continue
                product = line.product_id
                slot = prices.setdefault(product.name, {
                    'dt': order.date_order,
                    'price': unit,
                    'qty': 0.0,
                    'category': (product.categ_id.display_name
                                 if product.categ_id else 'غير مصنّف'),
                })
                slot['qty'] += line.qty
                if order.date_order >= slot['dt']:
                    slot['dt'] = order.date_order
                    slot['price'] = unit
        return prices

    @staticmethod
    def _collect_online_unit_prices(moves, discount_product_ids, prices):
        """يضيف آخر سعر وحدة من بنود فواتير المتجر الإلكتروني."""
        for m in moves:
            if m.move_type != 'out_invoice':
                continue
            d = m.invoice_date or m.date
            dt = datetime.combine(d, time.min) if d else datetime.min
            for line in m.invoice_line_ids:
                if line.display_type not in (False, 'product'):
                    continue
                product = line.product_id
                if not product or product.id in discount_product_ids:
                    continue
                if line.price_unit <= 0:
                    continue
                slot = prices.setdefault(product.name, {
                    'dt': dt, 'price': line.price_unit, 'qty': 0.0,
                    'category': (product.categ_id.display_name
                                 if product.categ_id else 'غير مصنّف'),
                })
                ...
                if dt >= slot['dt']:
                    slot['dt'] = dt
                    slot['price'] = line.price_unit
        return prices

    def _price_changed_products(self, cur_prices, prev):
        """المنتجات التي تغيّرت قيمة حقل «سعر الوحدة» لبنودها عن الشهر
        السابق فقط - يقارن آخر سعر وحدة مسجل في كل شهر كما هو في بنود
        البيع، فالخصومات لا تؤثر على النتيجة إطلاقاً. المنتجات ثابتة
        السعر والجديدة والمتوقفة لا تظهر."""
        prev_prices = prev.get('unit_prices') or {}
        rows = []
        for name, c in cur_prices.items():
            # استبعاد منتجات وفئات الخصم والتوصيل/الشحن
            if self._is_discount_name(name) \
                    or self._is_service_name(name) \
                    or self._is_discount_name(c.get('category')) \
                    or self._is_service_name(c.get('category')):
                continue
            p = prev_prices.get(name)
            if not p:
                continue
            diff = c['price'] - p['price']
            if abs(diff) < 0.01:
                continue
            pct = diff / p['price'] * 100.0 if p['price'] else 0.0
            if diff > 0:
                arrow, klass = '▲', 'ak-up'
            else:
                arrow, klass = '▼', 'ak-down'
            rows.append({
                'name': name,
                'category': c['category'],
                'qty_fmt': self._fmt_qty(c['qty']),
                'cur_price_fmt': self._fmt_amount(c['price']),
                'prev_price_fmt': self._fmt_amount(p['price']),
                'diff_fmt': '%s %s' % (arrow, self._fmt_amount(abs(diff))),
                'pct_fmt': '%{:+.1f}'.format(pct),
                'cls': klass,
                '_abs_pct': abs(pct),
            })
        rows.sort(key=lambda r: r['_abs_pct'], reverse=True)
        for i, r in enumerate(rows, start=1):
            r['rank'] = i
        return {'rows': rows, 'count': len(rows)}

    # --- إحصاءات المنتجات/الفئات لكل قناة بيع (كمية/مبلغ/خصم) ------------
    def _channel_product_stats(self, orders, discount_product_ids):
        """{اسم القناة: {'categories': {اسم الفئة: {qty, total,
        discount}}, 'products': {اسم المنتج: {...}}}} لقنوات نقاط
        البيع - بنفس منطق القسم الرابع (استبعاد بنود الخصم العام
        والمكافآت ومعالجة الكومبو) عبر استدعائه لكل قناة على حدة."""
        stats = {}
        for cfg in self.config_ids:
            ...
            ch = stats.setdefault(
                cfg.name, {'categories': {}, 'products': {}})
            for cat in p4['categories']:
                slot = ch['categories'].setdefault(
                    cat['name'], {'qty': 0.0, 'total': 0.0,
                                  'discount': 0.0, 'products': {}})
                slot['qty'] += cat['qty']
                slot['total'] += cat['total']
                slot['discount'] += self._raw_discount(cat)
                for p in cat['products']:
                    for pmap in (slot['products'], ch['products']):
                        pslot = pmap.setdefault(
                            p['name'],
                            {'qty': 0.0, 'total': 0.0, 'discount': 0.0})
                        pslot['qty'] += p['qty']
                        pslot['total'] += p['total']
                        pslot['discount'] += self._raw_discount(p)
        return stats

    @staticmethod
    def _add_online_product_stats(stats, cats, channel_name):
        """يضيف تجميعات منتجات/فئات المتجر الإلكتروني (كمية/مبلغ/خصم)
        إلى إحصاءات القنوات تحت اسم قناة الأونلاين."""
        ch = stats.setdefault(
            channel_name, {'categories': {}, 'products': {}})
        for cat in cats.values():
            slot = ch['categories'].setdefault(
                cat['name'], {'qty': 0.0, 'total': 0.0,
                              'discount': 0.0, 'products': {}})
    ...
    def _monthly_vip(self, sales_orders, old_partner_ids, prev):
        """عملاء VIP لكل قناة نقاط بيع على حدة: كل عميل تجاوز إنفاقه
        في القناة حد VIP_MIN_TOTAL، مع أعمدة المقارنة بالشهر السابق."""

        def build_rows(per):
            vips = sorted(
                (v for v in per.values()
                 if v['total'] > self.VIP_MIN_TOTAL),
            ...
            for i, v in enumerate(vips, start=1):
                ...
                row.update(self._cmp_vals(
                    v['total'],
                    prev['customers'].get(v['partner_id'], 0.0)))
                rows.append(row)
            return rows

        channels, total_customers = [], 0
        vip_partner_ids = set()
        for cfg in self.config_ids:
            per = {}
            for order in sales_orders:
                ...
                if not partner \
                        or order.session_id.config_id.id != cfg.id:
                    continue
                vals = per.setdefault(partner.id, {
                    'partner_id': partner.id,
                    'name': partner.name,
                    'phone': partner.mobile or partner.phone or '-',
                    'ptype': 'قديم' if partner.id in old_partner_ids
                             else 'جديد',
                    'total': 0.0, 'count': 0,
                })
                vals['total'] += order.amount_total
            ...
            vip_partner_ids |= {
                pid for pid, v in per.items()
                if v['total'] > self.VIP_MIN_TOTAL
                and not self._is_generic_customer(v['name'])}
            channels.append({
                'name': cfg.name,
                'rows': build_rows(per),
                'customer_count': len(per),
            })
        return {
            'channels': channels,
            'has_rows': any(c['rows'] for c in channels),
            'total_customers': total_customers,
            'vip_unique_count': len(vip_partner_ids),
            'threshold_fmt': self._fmt_amount(self.VIP_MIN_TOTAL),
        }

    # --- بطاقات العملاء العامة ------------------------------------------
    @staticmethod
    def _is_generic_customer(name):
        """بطاقات العملاء العامة المشتركة في قاعدة البيانات (عميل نقدي،
        عميل أونلاين، Walk-in...) ليست عملاء حقيقيين فتُستبعد من قسم
        العملاء المشتركين ومن عدادات الملخص."""
        n = (name or '').strip().lower()
        return any(tok in n for tok in (
            'نقدي', 'اونلاين', 'أونلاين', 'كاش',
            'cash', 'walk-in', 'walk in', 'guest', 'anonymous',
            'online customer', 'عميل عام'))
    ...
    def _build_charts(self, data):
        ...
        # 2) أعمدة اتجاه الأشهر (شاملة الأونلاين)
        tr = data['trend']['rows']
        ...
        def clip(name, limit=30):
            """قصّ الأسماء الطويلة في الرسوم حتى لا تلتف وتتداخل."""
            name = name or ''
        ...
        cats_chart = [{
            'name': clip(c['name'], 24), 'value_fmt': c['total_fmt'],
            'share_fmt': c['share_fmt'],
            'width': '{:.1f}%'.format(
                raw(c['total_fmt']) / mxc * 100.0 if mxc else 0.0),
        } for c in cats]
        # 4) أشرطة أعلى 10 منتجات
        prods = data['top_products']['rows'][:10]
        mxp = max((p.get('total', 0.0) for p in prods), default=0.0)
        prods_chart = [{
            'name': clip(p['name'], 30), 'value_fmt': p['total_fmt'],
            'width': '{:.1f}%'.format(
                p.get('total', 0.0) / mxp * 100.0 if mxp else 0.0),
        } for p in prods]
        # 5) شريطا أبرز مؤشرات الشهر
        highlights = self._dashboard_highlights(data)
        highlights2 = self._dashboard_highlights2(data)
        return {'channels': channels_chart, 'trend': trend_chart,
                'categories': cats_chart, 'products': prods_chart,
                'highlights': highlights, 'highlights2': highlights2}

    def _dashboard_highlights(self, data):
        """أبرز مؤشرات الشهر للوحة البصرية: أفضل قناة، أعلى فئة وأعلى
        منتج، متوسط المبيعات اليومي، ونمو صافي المبيعات عن الشهر
        السابق - كلها شاملة قناة المتجر الإلكتروني."""
        s1 = data['section1']['rows']
        best_ch = max(s1, key=lambda r: r.get('total', 0.0)) if s1 else None
        cats = data['category_summary']['rows']
        prods = data['top_products']['rows']
        first = self.month_start.replace(day=1)
        days = (self._month_last_day() - first).days + 1
        net = data['section1'].get('grand_total', 0.0)
        growth = next(
            (r for r in data['channel_comparison']['rows']
             if r['label'].startswith('صافي')), None)
        items = []
        if best_ch:
            items.append({
                'label': 'أفضل قناة بيع',
                'value': best_ch['name'],
                'sub': '%s من المبيعات' % best_ch['share_fmt'],
                'is_amount': False, 'cls': '',
            })
        if cats:
            items.append({
                'label': 'أعلى فئة مبيعاً',
                'value': cats[0]['name'],
                'sub': cats[0]['total_fmt'] + ' ر.س',
                'is_amount': False, 'cls': '',
            })
        if prods:
            pname = prods[0]['name'] or ''
            if len(pname) > 28:
                pname = pname[:27] + '…'
            items.append({
                'label': 'أعلى منتج مبيعاً',
                'value': pname,
                'sub': prods[0]['total_fmt'] + ' ر.س',
                'is_amount': False, 'cls': '',
            })
        items.append({
            'label': 'متوسط المبيعات اليومي',
            'value': self._fmt_amount(net / days if days else 0.0),
            'sub': 'صافي / %d يوماً' % days,
            'is_amount': True, 'cls': '',
        })
        if growth:
            items.append({
                'label': 'نمو صافي المبيعات عن الشهر السابق',
                'value': growth['pct_fmt'],
                'sub': growth['diff_fmt'],
                'is_amount': True, 'cls': growth['class'],
            })
        return items

    @staticmethod
    def _is_service_name(name):
        """فئات/منتجات الخدمات اللوجستية (التوصيل والشحن) تُستبعد من
        تحليلات الملخص التنفيذي - ليست منتجات بيع فعلية."""
        n = (name or '').strip().lower()
        return any(tok in n for tok in
                   ('delivery', 'توصيل', 'شحن', 'shipping'))

    @staticmethod
    def _ar_count(n, singular, dual, plural, acc):
        """صياغة عربية سليمة للأعداد: 1 مفرد، 2 مثنى، 3-10 جمع،
        11+ تمييز منصوب."""
        if n == 1:
            return singular
        if n == 2:
    ...
    def _executive_summary(self, data):
        """الملخص التنفيذي: بطاقات مبوبة (الأداء العام / قنوات البيع /
        فئات المنتجات / العملاء / منتجات فاقت ودون التوقعات / ملاحظات)
        تُبنى آلياً من أرقام التقرير وتُعرض في الصفحة الأولى."""
        def raw(fmt):
            try:
                return float(str(fmt).replace(',', '').replace(
                    '%', '').replace('+', ''))
            except (TypeError, ValueError):
                return 0.0

        perf, channels, cats_g, customers, notes = [], [], [], [], []

        growth = next(
            (r for r in data['channel_comparison']['rows']
             if r['label'].startswith('صافي')), None)
        if growth and growth['previous_fmt'] != '—' \
                and raw(growth['previous_fmt']):
            verb = ('ارتفع' if growth['class'] == 'ak-up'
                    else 'انخفض' if growth['class'] == 'ak-down'
                    else 'استقر')
            perf.append({
                'text': '%s صافي المبيعات بنسبة %s عن الشهر السابق، '
                        'بفارق قدره %s ر.س.' % (
                            verb, growth['pct_fmt'], growth['diff_fmt']),
                'cls': growth['class']})
        gross = raw(data['kpis'].get('gross_fmt'))
        refunds = raw(data['kpis'].get('refunds_fmt'))
        if gross:
            pct = refunds / gross * 100.0
            high = pct > 5.0
            perf.append({
                'text': 'بلغت نسبة المرتجعات %%%.1f من إجمالي '
                        'المبيعات، وهي %s.' % (
                            pct, 'أعلى من الحد الصحي (5%)'
                            if high else 'ضمن الحدود الصحية'),
                'cls': 'ak-down' if high else 'ak-up'})
            if high:
                notes.append({
                    'text': 'نسبة المرتجعات المرتفعة تستدعي مراجعة '
                            'أسبابها مع إدارة الفروع.',
                    'cls': 'ak-down'})

        rows1 = data['section1']['rows']
        if rows1:
            best = max(rows1, key=lambda r: r.get('total', 0.0))
            channels.append({
                'text': 'تصدّرت قناة %s المبيعات بحصة %s من صافي '
                        'مبيعات الشهر.' % (best['name'],
                                           best['share_fmt']),
                'cls': 'ak-up'})
            if len(rows1) > 1:
                worst = min(rows1, key=lambda r: r.get('total', 0.0))
                channels.append({
                    'text': 'سجلت قناة %s أدنى مساهمة بحصة %s، '
                            'ويُوصى بخطة تنشيط لها.' % (
                                worst['name'], worst['share_fmt']),
                    'cls': ''})

        # فئات ومنتجات التقرير مع أرقام الشهر السابق (من قسم التفاصيل)،
        # مع استبعاد فئات الخدمات (التوصيل/الشحن) وفئات الخصم
        pd_cats = [
            c for c in data['products_detail']['categories']
            if not self._is_service_name(c['name'])
            and not self._is_discount_name(c['name'])]

        def cat_pct(c):
            return raw(c.get('pct_fmt'))

        def drivers(cat, declining=False):
            """أبرز المنتجات المسببة لنمو/تراجع الفئة."""
            scored = []
            for r in cat.get('rows') or []:
                if self._is_service_name(r['name']):
                    continue
                delta = raw(r.get('total_fmt')) - raw(r.get('prev_fmt'))
                scored.append((delta, r['name']))
            scored.sort(reverse=not declining)
            picked = [n for d, n in scored[:2]
                      if (d < 0 if declining else d > 0)]
            return '، '.join(picked)

        measurable = [c for c in pd_cats
                      if c.get('pct_fmt') not in (None, '—')
                      and raw(c.get('prev_fmt'))]
        ups = [c for c in measurable if '+' in c['pct_fmt']]
        downs = sorted((c for c in measurable if '-' in c['pct_fmt']),
                       key=cat_pct, reverse=True)
        if ups:
            top = max(ups, key=cat_pct)
            lead = drivers(top)
            cats_g.append({
                'text': 'حققت فئة %s أعلى نمو بنسبة %s عن الشهر '
                        'السابق%s.' % (
                            top['name'], top['pct_fmt'],
                            '، يقوده: %s' % lead if lead else ''),
                'cls': 'ak-up'})
        for c in downs[:3]:
            cause = drivers(c, declining=True)
            cats_g.append({
                'text': 'تراجعت فئة %s بنسبة %s%s.' % (
                    c['name'], c['pct_fmt'],
                    '، وأبرز أسباب التراجع: %s' % cause
                    if cause else '، وتستحق مراجعة المخزون والتسعير'),
                'cls': 'ak-down'})

        new_c = data['section1']['total'].get('new')
        if new_c:
            customers.append({
                'text': 'انضم %s جديداً خلال الشهر عبر قنوات نقاط '
                        'البيع.' % self._ar_count(
                            new_c, 'عميل واحد', 'عميلان',
                            'عملاء', 'عميلاً'),
                'cls': 'ak-up'})
        vip = data.get('vip') or {}
        if vip.get('vip_unique_count'):
            customers.append({
                'text': 'تجاوز إنفاق %s حد %s ر.س خلال الشهر - '
                        'تفاصيلهم في قسم عملاء VIP.' % (
                            self._ar_count(
                                vip['vip_unique_count'], 'عميل واحد',
                                'عميلين', 'عملاء', 'عميلاً'),
                            vip.get('threshold_fmt', '5,000.00')),
                'cls': 'ak-up'})

        # منتجات فاقت التوقعات / دون التوقعات (نمو وتراجع استثنائيان)
        overs, unders = [], []
        prod_stats = []
        for c in pd_cats:
            for r in c.get('rows') or []:
                if self._is_service_name(r['name']) \
                        or self._is_discount_name(r['name']):
                    continue
                cur_v, prev_v = raw(r.get('total_fmt')), raw(
                    r.get('prev_fmt'))
                if prev_v < 300.0 or r.get('pct_fmt') in (None, '—'):
                    continue
                pct_v = raw(r['pct_fmt'])
                if pct_v < 25.0:  # يُعرض التغير الاستثنائي فقط (±25%)
                    continue
                prod_stats.append(
                    (pct_v, '+' in r['pct_fmt'], r, cur_v, prev_v))
        for pct, is_up, r, cur_v, prev_v in sorted(
                (p for p in prod_stats if p[1]),
                key=lambda x: x[0], reverse=True)[:3]:
            overs.append({
                'text': '%s: نمو %s (من %s إلى %s ر.س).' % (
                    r['name'], r['pct_fmt'], r['prev_fmt'],
                    r['total_fmt']),
                'cls': 'ak-up'})
        for pct, is_up, r, cur_v, prev_v in sorted(
                (p for p in prod_stats if not p[1]),
                key=lambda x: x[0], reverse=True)[:3]:
            unders.append({
                'text': '%s: تراجع %s (من %s إلى %s ر.س).' % (
                    r['name'], r['pct_fmt'], r['prev_fmt'],
                    r['total_fmt']),
                'cls': 'ak-down'})

        pc = data.get('price_changes') or {}
        if pc.get('count'):
            notes.append({
                'text': 'تغيّر سعر %s عن الشهر السابق - التفاصيل في '
                        'قسم المنتجات المتغيرة الأسعار.'
                        % self._ar_count(
                            pc['count'], 'منتج واحد', 'منتجين',
                            'منتجات', 'منتجاً'), 'cls': ''})
        if not notes:
            notes.append({
                'text': 'لا توجد ملاحظات تستدعي تدخلاً عاجلاً هذا '
                        'الشهر.', 'cls': 'ak-up'})

        groups = [g for g in [
            {'title': 'الأداء العام', 'lines': perf},
            {'title': 'قنوات البيع', 'lines': channels},
            {'title': 'فئات المنتجات', 'lines': cats_g},
            {'title': 'العملاء', 'lines': customers},
            {'title': 'منتجات فاقت التوقعات', 'lines': overs},
            {'title': 'منتجات دون التوقعات', 'lines': unders},
            {'title': 'ملاحظات لعناية الإدارة', 'lines': notes},
        ] if g['lines']]
        card_rows = [groups[i:i + 2] for i in range(0, len(groups), 2)]
        return {'card_rows': card_rows}

    def _dashboard_highlights2(self, data):
        """الشريط الثاني: أعلى موظف مبيعاً في كل قناة نقاط بيع (بدون
        المتجر الإلكتروني)، أفضل أسبوع في الشهر، ونسبة المرتجعات."""
        def raw(fmt):
            try:
                return float(str(fmt).replace(',', ''))
            except (TypeError, ValueError):
        ...
        items = []
        for cfg_block in data['section5']['configs']:
            rows = cfg_block.get('rows') or []
            if not rows:
                continue
            top = max(rows, key=lambda r: r.get('total', 0.0))
            items.append({
                'label': 'أعلى موظف مبيعاً — %s' % cfg_block['name'],
                'value': top['name'],
                'sub': top['total_fmt'] + ' ر.س',
                'is_amount': False, 'cls': '',
            })
        best_week = next(
            (r for r in data['weekly_rows']['rows'] if r.get('is_best')),
            None)
        if best_week:
            items.append({
                'label': 'أفضل أسبوع في الشهر',
                'value': best_week['label'].split(':')[0],
                'sub': best_week['total_fmt'] + ' ر.س',
                'is_amount': False, 'cls': '',
            })
        gross = raw(data['kpis'].get('gross_fmt'))
        refunds = raw(data['kpis'].get('refunds_fmt'))
        if gross:
            pct = refunds / gross * 100.0
            items.append({
                'label': 'نسبة المرتجعات من المبيعات',
                'value': '%{:.1f}'.format(pct),
                'sub': refunds and (
                    data['kpis']['refunds_fmt'] + ' ر.س') or 'لا مرتجعات',
                'is_amount': True,
                'cls': 'ak-down' if pct > 5.0 else '',
            })
        return items
    ...
    def action_print_report(self):
        ...
        # منع تراكم النسخ في قاعدة البيانات: تحذف النسخ السابقة لنفس
        # الشهر (نفس الاسم) التي أنشأها هذا المعالج قبل إنشاء الجديدة
        Attachment.search([
            ('name', '=', filename),
