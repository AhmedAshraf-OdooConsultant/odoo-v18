...
class InventoryAgeBreakdownReport(models.TransientModel):
    ...
    def get_location_domain(self):
        ...
        product_locations = []
        if self.product_ids:
            product_locations = list(set([product.property_stock_inventory.id for product in self.env["product.product"].browse(self.product_ids.ids)]))
        if product_locations:
            domain += [('id', 'in', product_locations)]
    ...
    def get_report_data(self):
        ...
        if result_data:
            data = {"result_data": result_data, "main_header": main_header}
            return data
        else:
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            "inventory_age_report.report_inventory_age_breakdown_action"
    ...
    def action_excel(self):
        ...
        return {
            ...
            "tag": "inventory_age_report.report_inventory_breakdown_xlsx",
            "data": {
                "model": "inventory.age.breakdown.report",
                "options": json.dumps(data, default=tools.json_default),
                "output_format": "xlsx",
    ...
    def get_xlsx_report(self, data, response):
        ...
        cell_format = workbook.add_format({"font_size": "12px", "align": "left"})
        ...
        for col, header in enumerate(main_headers):
            sheet.merge_range(7, col * 2 + 4, 7, col * 2 + 5, header, header_style)
